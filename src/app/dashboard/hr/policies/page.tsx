...
} from "@/features/hr/actions/policy.actions";
...
export default function HrPoliciesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isCreateMode) {
      ...
      setLoading(false);
      if (res.success && res.data) {
        setMessage({ type: "success", text: "Dokumen kebijakan baru berhasil disimpan!" });
        await loadPolicies((res.data as any).id);
      } else {
        setMessage({ type: "error", text: res.error || "Gagal membuat dokumen kebijakan baru." });
      }
    } else {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    setMessage(null);
    const res = await deletePolicyAction(selectedPolicy.id);
    setLoading(false);

    if (res.success) {
      setMessage({ type: "success", text: "Dokumen kebijakan berhasil dihapus!" });
      await loadPolicies();
    } else {
      setMessage({ type: "error", text: res.error || "Gagal menghapus dokumen kebijakan." });
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      const res = await extractDocumentTextAction(base64, file.name);
      ...
      if (res.success && res.data) {
        ...
        setMessage({ type: "success", text: `Dokumen ${file.name} berhasil diekstrak dengan Groq LLM!` });
      } else {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-900 pb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Lihat, tambah, edit, dan hapus basis pengetahuan AI HR Copilot sesuai Peraturan Ketenagakerjaan RI Juli 2026.
          </p>
        </div>
        <div>
          <Link
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 rounded-2xl border border-zinc-900 bg-zinc-900/10 p-4 h-[70vh] flex flex-col">
          <div className="flex justify-between items-center mb-4 px-2">
            <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Daftar Dokumen ({policies.length})</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {policies.length === 0 ? (
              <p className="text-zinc-600 text-xs text-center py-8">Belum ada dokumen kebijakan.</p>
            ) : (
              policies.map((p) => {
          ...
          <form onSubmit={handleSave} className="flex-1 flex flex-col space-y-4">
            <div className="flex justify-between items-center gap-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider text-zinc-400">
                {isCreateMode ? "Tambah Kebijakan Baru" : "Sunting Dokumen Editor"}
              </h3>
              {message && (
                <span
            ...
            {isCreateMode && (
              <div className="p-3.5 border border-dashed border-zinc-800 rounded-xl bg-zinc-950/40 flex items-center justify-between gap-4">
                <div>
                  <h4 className="text-xs font-semibold text-zinc-300">Ekstrak Otomatis via AI (Groq LLM)</h4>
                  <p className="text-[10px] text-zinc-500 mt-0.5">Unggah berkas PDF, TXT, atau DOCX. AI akan melengkapi formulir di bawah secara instan.</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2">
                ...
                <input
                  ...
                  placeholder="Contoh: Kebijakan Lembur Juli 2026"
                />
              </div>
              <div>
                <label className="block text-[9px] font-bold uppercase tracking-wider text-zinc-500 mb-1.5">
            ...
            <div className="flex justify-between items-center pt-4 border-t border-zinc-900">
              ...
              <div className="flex gap-2">
                ...
                >
                  {loading ? "Menyimpan..." : isCreateMode ? "Simpan Dokumen Baru" : "Simpan Perubahan RAG"}
                </button>
              </div>
            </div>
          </form>
